...
//		  newOrder.setStatus(OrderStatus.PAID); } else {
//
//			newOrder.setStatus(OrderStatus.NEW);
//		}
	...
	public void setOrderReturnRequested(OrderReturnRequest request, Customer customer)
		...
		track.setUpdatedTime(new Date());
		track.setStatus(OrderStatus.RETURN_CANCEL_REQUESTED);
		...
		order.setStatus(OrderStatus.RETURN_CANCEL_REQUESTED);

		repo.save(order);
	}

	public void processOrder(Order order) {
		// Retrieve the products ordered and update the stock count
		...
		for (OrderDetail orderDetail : order.getOrderDetails()) {
			...
			if (product != null) {
				int  currentStockCount = Integer.parseInt(product.getStock());
				int orderedQuantity = orderDetail.getQuantity();
				int updatedStockCount = currentStockCount - orderedQuantity;
				product.setStock(String.valueOf(updatedStockCount));
